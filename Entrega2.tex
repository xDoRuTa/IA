...
A diferencia de una b\'usqueda \textit{First Improvement}, el algoritmo \textbf{no aplica el primer movimiento que mejora}, sino que conserva el m\'inimo global de $\Delta$ entre absolutamente todas las rutas, dep\'ositos y posiciones evaluadas en esa pasada, y solo entonces ejecuta el \'unico movimiento con mayor reducci\'on (\textbf{Best Improvement} a nivel de soluci\'on completa, no solo de ruta). El ciclo se repite hasta que ninguna perturbaci\'on produce $\Delta < -10^{-4}$, o hasta alcanzar el l\'imite de 1000 iteraciones. Es importante notar una asimetr\'ia entre operadores: para \texttt{RELOCATE} se eval\'uan todas las posiciones de inserci\'on $j \neq i$ (incluyendo posiciones anteriores a $i$), mientras que para \texttt{SWAP} y \texttt{2-OPT} solo se consideran pares con $j > i$, ya que intercambiar $(i,j)$ es equivalente a intercambiar $(j,i)$ y revertir el segmento $[i,j]$ es id\'entico a revertirlo en sentido contrario; esto evita evaluar movimientos sim\'etricamente redundantes sin perder cobertura de la vecindad.

\textbf{Relocate Inter-Ruta.} Tras converger el HC intra-ruta, se invoca repetidamente la funci\'on \texttt{relocate\_inter\_ruta} mediante el patr\'on \texttt{while (relocate\_inter\_ruta(...))}. A diferencia del operador intra-ruta, esta funci\'on eval\'ua, en una sola pasada, el costo de \textbf{remover} cada cliente $i$ de su ruta de origen $(d_1, v_1)$ -- calculando la ganancia $g = \text{FO}(\text{ruta}_1) - \text{FO}(\text{ruta}_1 \setminus \{i\})$ -- y el costo de \textbf{insertarlo} en cada posici\'on $j$ de cualquier otra ruta $(d_2, v_2)$ del sistema, incluyendo rutas de otros dep\'ositos -- calculando el costo $c = \text{FO}(\text{ruta}_2 \cup \{i\}) - \text{FO}(\text{ruta}_2)$. El movimiento global se acepta solo si $\delta = c - g < 0$, y de todas las combinaciones $(i, d_1, v_1, j, d_2, v_2)$ evaluadas se ejecuta \'unicamente la de menor $\delta$. Si ninguna combinaci\'on mejora la soluci\'on, la funci\'on retorna \texttt{false} y el ciclo \texttt{while} en el \textit{main} se detiene. Este operador es, en la pr\'actica, el \'unico mecanismo capaz de mover clientes \textbf{entre dep\'ositos distintos}, lo que lo convierte en el verdadero responsable de resolver la dimensi\'on ``multi-dep\'osito'' del problema; los tres operadores intra-ruta descritos en la Secci\'on~6.4 operan exclusivamente dentro de una \'unica ruta y son, por tanto, ciegos a la asignaci\'on cliente-dep\'osito.
...
Cada evaluaci\'on de movimiento, sin importar el operador, requiere clonar la ruta completa (\texttt{vector<Node*> vec\_ruta = ruta;}) y volver a calcular su distancia y tiempo desde el dep\'osito mediante \texttt{evaluar\_ruta}, en $O(N)$ por evaluaci\'on. Como existen $O(N^2)$ pares $(i,j)$ por ruta, el costo de explorar la vecindad completa de \textbf{una} ruta en una sola iteraci\'on de HC es $O(N^3)$; sumado sobre todas las rutas y dep\'ositos, y repetido hasta convergencia, este es precisamente el cuello de botella algor\'itmico que se retoma cuantitativamente en la Secci\'on~8 y se discute como limitaci\'on central en las Conclusiones.
